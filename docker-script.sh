...
docker container run --rm -it -u $(id -u ${USER}):$(id -g ${USER}) --name MozLz4-decompress -v "$PWD":/usr/src/myapp -v "$PWD/.local":"/.local" -v "$firefoxBackupFolder":/usr/src/bookmarksFolder -w /usr/src/myapp -e PYTHONDONTWRITEBYTECODE=1 python:3.11.4-bookworm /bin/bash -l -c "pip install --no-cache-dir lz4 && python /usr/src/myapp/mozLz4-decompress/mozlz4.py -d /usr/src/bookmarksFolder/$lastUpdatedFile /usr/src/myapp/output/bookmarks.json"
docker container run --rm -it -u $(id -u ${USER}):$(id -g ${USER}) --name exportToMarkdown  -v "$PWD":/usr/src/myapp -v "$PWD/.local":"/.local" -v "$firefoxBackupFolder":/usr/src/bookmarksFolder -w /usr/src/myapp -e PYTHONDONTWRITEBYTECODE=1 python:3.11.4-bookworm /bin/bash -l -c "python3 exportToMarkdown.py /usr/src/myapp/output/bookmarks.json" > "$outputFolder/bookmarks.md"
